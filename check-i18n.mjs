// Every data-i18n* key in the markup must exist in all four dictionaries,
...
for (const [, lang, body] of html.matchAll(/^\s{2}(en|af|zu|xh):\s*\{([\s\S]*?)^\s{2}\}/gm)) {
  // keys are packed several per line, so match `key:'` anywhere, not line-anchored
  dicts[lang] = new Set([...body.matchAll(/([a-z0-9_]+)\s*:\s*["']/gi)].map(m => m[1]));
}

let bad = 0;
for (const lang of ['en', 'af', 'zu', 'xh']) {
  const dict = dicts[lang];
  if (!dict) { console.error(`FAIL: no ${lang} dictionary found`); bad++; continue; }
  const missing = [...used].filter(k => !dict.has(k));
  if (missing.length) { console.error(`FAIL ${lang}: missing ${missing.length} — ${missing.join(', ')}`); bad++; }
  else console.log(`ok ${lang}: all ${used.size} keys present`);
}
